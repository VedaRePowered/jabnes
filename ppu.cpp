		...
		for (unsigned ppu_clock = last_change_pixel; ppu_clock < change_pixel; ppu_clock+=8) {
			...
			if (pixel.colliding) {
				...
				if (!(ppu_control & 0b00010000)) { // pattern table select (0x0000 or 0x1000)
					palette_colour_high_byte = current_state.get_ppu_memory((tile<<4)+(pixel.y&7)-ppu_scroll_y);
					palette_colour_low_byte = current_state.get_ppu_memory((tile<<4)+(pixel.y&7)-ppu_scroll_y+8);
				} else {
					palette_colour_high_byte = current_state.get_ppu_memory((tile<<4)+(pixel.y&7)-ppu_scroll_y+0x1000);
